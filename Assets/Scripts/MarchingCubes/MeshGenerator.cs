...
{
    static readonly private List<Vector3> verts = new();
    static readonly private List<Color32> colours = new();
    static readonly private List<Vector3> normals = new();
    static readonly private List<int> tris = new();

    static private int size = 1;

    static public MeshData GenerateMeshDataForChunkData(ChunkData chunkData, float isoLevel, float rainbowLength, float saturation, float value)
    {
        ...
        {
            int avgTrisPerCube = 2;
            int cubeCount = (size - 1) * (size - 1) * (size - 1);
            int vertCount = cubeCount * avgTrisPerCube;
            verts.Capacity = vertCount;
            colours.Capacity = vertCount;
            normals.Capacity = vertCount;
            tris.Capacity = vertCount;
        }
        ...
        for (int x = 0; x < size - 1; ++x)
        {
            for (int y = 0; y < size - 1; ++y)
            {
                ...
                {
                    Vector4[] cubeCorners = {
                        chunkData.data[PosToIndex(x, y, z)],
                        chunkData.data[PosToIndex(x + 1, y, z)],
                        chunkData.data[PosToIndex(x + 1, y, z + 1)],
                        chunkData.data[PosToIndex(x, y, z + 1)],
                        chunkData.data[PosToIndex(x, y + 1, z)],
                        chunkData.data[PosToIndex(x + 1, y + 1, z)],
                        chunkData.data[PosToIndex(x + 1, y + 1, z + 1)],
                        chunkData.data[PosToIndex(x, y + 1, z + 1)],
                    };
                    ...
                    bool interpVerts = true;
                    if (interpVerts)
                    {
                        //Find the vertices where the surface intersects the cube
                        if ((edges & 1) == 1)       edgePoints[0] =  VertexInterp(cubeCorners[0], cubeCorners[1], isoLevel);
                        if ((edges & 2) == 2)       edgePoints[1] =  VertexInterp(cubeCorners[1], cubeCorners[2], isoLevel);
                        if ((edges & 4) == 4)       edgePoints[2] =  VertexInterp(cubeCorners[2], cubeCorners[3], isoLevel);
                        if ((edges & 8) == 8)       edgePoints[3] =  VertexInterp(cubeCorners[3], cubeCorners[0], isoLevel);
                        if ((edges & 16) == 16)     edgePoints[4] =  VertexInterp(cubeCorners[4], cubeCorners[5], isoLevel);
                        if ((edges & 32) == 32)     edgePoints[5] =  VertexInterp(cubeCorners[5], cubeCorners[6], isoLevel);
                        if ((edges & 64) == 64)     edgePoints[6] =  VertexInterp(cubeCorners[6], cubeCorners[7], isoLevel);
                        if ((edges & 128) == 128)   edgePoints[7] =  VertexInterp(cubeCorners[7], cubeCorners[4], isoLevel);
                        if ((edges & 256) == 256)   edgePoints[8] =  VertexInterp(cubeCorners[0], cubeCorners[4], isoLevel);
                        if ((edges & 512) == 512)   edgePoints[9] =  VertexInterp(cubeCorners[1], cubeCorners[5], isoLevel);
                        if ((edges & 1024) == 1024) edgePoints[10] = VertexInterp(cubeCorners[2], cubeCorners[6], isoLevel);
                        if ((edges & 2048) == 2048) edgePoints[11] = VertexInterp(cubeCorners[3], cubeCorners[7], isoLevel);
                    }

                    // Add the tris with the correct vertex index
                    int[] currentTris = triTable[cubeIndex].Clone() as int[];
                    float avgY = 0;
                    for (int i = 0; i < currentTris.Length; i++)
                    {
                        tris.Add(verts.Count);
                        verts.Add(edgePoints[currentTris[i]]);
                        currentTris[i] = verts.Count - 1;
                        normals.Add(Vector3.zero);
                        avgY += verts[^1].y;
                        if (i % 3 == 2)
                        {
                            avgY *= 0.333333f;
                            float hue = Mathf.Abs(avgY % rainbowLength) / rainbowLength;
                    ...
                    {
                        normals[currentTris[i]] = normals[currentTris[i + 1]] = normals[currentTris[i + 2]] = Vector3.Normalize(Vector3.Cross(verts[currentTris[i + 1]] - verts[currentTris[i]], verts[currentTris[i + 2]] - verts[currentTris[i]]));
                    }
                }
            }
        }

        meshData.verts = verts.ToArray();
        meshData.normals = normals.ToArray();
        meshData.tris = tris.ToArray();
        meshData.colours = colours.ToArray();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector3 VertexInterp(Vector4 p1, Vector4 p2, float isoLevel)
    {
        float t = (isoLevel - p1.w) / (p2.w - p1.w);
        return (Vector3)(p1 + (t * (p2 - p1)));
    }


    static readonly private int[] edgeTable = {
    ...
    static readonly private int[][] triTable = new int[][]
    {new int[]{},
    new int[]{0, 8, 3},
    new int[]{0, 1, 9},
    new int[]{1, 8, 3, 9, 8, 1},
    ...
    //static readonly private int[] cornerIndexAFromEdge = {0,1,2,3,4,5,6,7,0,1,2,3};

    //static readonly private int[] cornerIndexBFromEdge = {1,2,3,0,5,6,7,4,4,5,6,7};
}
